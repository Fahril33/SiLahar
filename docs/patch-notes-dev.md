...
### Added
- Opsi pilihan **Tim** (TRC & PUSDALOPS) pada form laporan dengan pemisahan konfigurasi data pejabat koordinator penandatangan default di panel Admin.
- Trigger PostgreSQL (`019_enforce_report_date_rules_in_trigger.sql`) sebagai proteksi lapis akhir di level database terhadap manipulasi tanggal laporan.
- Tooltip informatif interaktif di samping label "Tanggal laporan" yang menyesuaikan status aturan dan mode pengguna (Admin / Publik).

### Changed
- Datepicker tanggal laporan dinonaktifkan (*disabled*) secara otomatis saat aturan `allow_any_report_date` bernilai nonaktif (`false`) pada database.
- Sinkronisasi aturan laporan dimuat langsung dari tabel `app_settings` secara *real-time* tanpa konfigurasi statis/dummy di sisi klien.
- Komponen Catatan Rilis disempurnakan dengan antarmuka timeline modern, badge kategori warna, dan proteksi akses khusus sesi Admin aktif.

### Fixed
- Perbaikan *stale date bug* pada SPA saat aplikasi terbuka melewati tengah malam dengan penentuan tanggal WITA dinamis.
- Perbaikan konsistensi peran pejabat approver (`coordinator_team_trc` dan `coordinator_team_pusdalops`) pada pembuatan *draft* kosong maupun pemulihan riwayat laporan.
